...
for i in 1:length(idx)
    ...
    global rmsd_ctv_idx = readdlm(joinpath(dir_ctv_pdt, string(phs[i]),
            string("rmsd_ctv_", idx[i])))[2:end, 2]
    global rmsd_ctv_ph = readdlm(joinpath(dir_ctv_all_pdt, string(phs[i]),
            string("rmsd_ctv_", phs[i])))[2:end, 2]

    sym_rmsd_ctv_idx = Symbol("rmsd_ctv_", idx[i])
    sym_rmsd_ctv_ph = Symbol("rmsd_ctv_", PH)
